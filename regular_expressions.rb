...
require 'pry'
@proper_format_regex = /\b\d{3}-\d{3}-\d{3}\b/
def has_sin?(string)
  #finds all the sins number usings /(\d{3}-\d{3}-\d{3})?/
  @proper_format_regex.match(string) ? true : false
...
def grab_sin(string)
  #match with the proper formatting of regex and search the strings
  match = @proper_format_regex.match(string)
  match ? match[0] : nil
...
def grab_all_sins(string)
  string.scan(@proper_format_regex)
...
def format_sins(string)
  new_string = string.gsub(/(\d{3})\.?(\d{3})\.?(\d{3})/, '\1-\2-\3')
end
